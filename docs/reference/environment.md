...
| :--- | :---------- | 
| **SHIFT_PATH_WORKFLOWS** | Directory paths set in this variable will be considered as workflow project locations. This means that workflows existing in these directories will use relative paths when being referenced in [Workflow](nodes/workflow#workflow-node) and [WorkflowProcess](nodes/workflow#workflowprocess-node) nodes.|
| **SHIFT_ACTIVATION_SCRIPT** | This variable should to an activation script to be run before a workflow is executed in  [Batch](../getting_started/basics/batch.md) mode. 
